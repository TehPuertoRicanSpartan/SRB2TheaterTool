...
class ToolState extends FlxState
{
	...
	function convertMovie(file:String, fps:Int, outWidth:Int, prefix:String, audioFile:String)
	{
		try {
			...
			if (file.startsWith("https://")) {
				Sys.command('mkdir ./temp');
				Sys.command('attrib +h /s /d ./temp');
				if (file.startsWith("https://youtube.com") || file.startsWith("https://youtu.be")) {
					Sys.command("./yt-dlp.exe", ['-P', './temp', '-o', '"%(title)s.%(ext)s"', file]);
					var title:String = Util.getCommand("./yt-dlp.exe", ['--print', 'filename', '-o', '"%(title)s.%(ext)s"', file]);
					file = Util.getCommand("cd") + "/temp/" + title;
				} else {
					Sys.command('wget -P ./temp/ "$file"');
			...
			Sys.command('zip -r "./movies/${luaID.text}.pk3" "./movies/${luaID.text}"');
	...
	{
		FlxG.sound.play('${path}sounds/DSS3K35.ogg');
	}
