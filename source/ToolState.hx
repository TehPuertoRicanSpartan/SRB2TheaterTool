...
class ToolState extends FlxState
{
	...
	function convertMovie(file:String, fps:Int, outWidth:Int, prefix:String, audioFile:String)
	{
		...
		Sys.command('mkdir "movies/${luaName.text}/Music"');
		Sys.command('mkdir "movies/${luaName.text}/Textures"');
		Sys.command('mkdir "movies/${luaName.text}/Lua"');
		...
		Sys.command('./ffmpeg.exe', [
			...
			'-start_number',
			'0',
			'movies/${luaName.text}/Textures/$prefix%0${numberLength}d.png',
			'-vn',
			'-acodec',
			'libvorbis',
			'movies/${luaName.text}/Music/O_$audioFile.ogg'
		]);
		...
		var frames:Int = 0;
		var items:Array<String> = FileSystem.readDirectory('./movies/${luaName.text}/Textures');
		for (item in items)
			frames++;

		var luaFile:String = '/* Automatically generated by SRB2TheaterTool v${Application.current.meta.get('version')} -- https://github.com/TehPuertoRicanSpartan/SRB2TheaterTool */ theater.addMovie{id = "${luaID.text}", name = "${luaName.text}", duration = $duration, width = $outputWidth, height = $height, textures = "$prefixWithAsteriks", frames = ${frames - 1}, music = "${audioName.text}"}';
		Util.writeTextToFile(luaFile, './movies/${luaName.text}/Lua/movie.lua');
	}
